...
def sendInterface(MemberList, MemberCount):
    ...
    print(u'我要对%s说：'%name),
    msg = input()
    print(u'希望在多少小时后发送？如果你输入0，则立即发送',)
    delay = float(input())
    sleep = delay*3600
    thread_res = []
    if(choice == 1):
...
def main():
    ...
    for t in threads:
        ...
        t.start()
        try:
            print('当前发送的是第%d个人:%s' % (mn+1,NickName[mn]))
        except IndexError:
            break
